...
긴 과제 안내문을 붙여넣으면 제출 체크리스트로 변환하는 **단일 페이지 MVP**입니다.
...
1차 사용자 인터뷰에서 "요구사항은 뽑히지만 우선순위/마감관리가 부족하다"는
피드백이 나오면 아래 중 **1개만** 추가하여 다시 검증합니다:
...
GA 태그를 `index.html`의 `<head>`에 추가하면 현재 코드의 `track()` 이벤트가 자동 기록됩니다.
